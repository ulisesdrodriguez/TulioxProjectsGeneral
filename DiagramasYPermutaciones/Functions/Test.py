        ...
        for j in range(len(imagen1[0])):
            if imagen1[i][j] == imagen2[i][j]:
                if validar_en_tercera and imagen3:
                    if len(imagen3) != len(imagen1) or len(imagen3[0]) != len(imagen1[0]):
                        raise ValueError("imagen3 debe tener el mismo tamaño que imagen1 e imagen2")
...
# Análisis con patrones específicos (Código Secuencia F)
resultados_patrones = buscar_patrones_especificos(imagen1, PATRONES_ESPECIFICOS)
